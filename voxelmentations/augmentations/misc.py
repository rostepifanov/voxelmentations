...
def _inrange_zero_one_number(param, name):
    if param < 0:
        raise ValueError('{} should be greater or equal zero.'.format(name))

    if param > 1:
        raise ValueError('{} should be less or equal one.'.format(name))
...
prepare_inrange_zero_one_float = lambda param, name: _prepare_float(param, name, _inrange_zero_one_number)
